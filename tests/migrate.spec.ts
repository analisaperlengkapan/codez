...
test.describe('Repository Migration Feature', () => {
  test('should migrate a repository and display imported data', async ({ page }) => {
    ...
    await page.getByPlaceholder('Repository Name').fill('migrated-repo');
    ...
    await page.waitForURL('http://127.0.0.1:8080/repos/admin/migrated-repo');
    await expect(page.getByRole('heading', { name: 'admin / migrated-repo' })).toBeVisible();

    // Wait for leptos router to settle
    await page.waitForTimeout(2000);

    // Verify mock issues were imported correctly
    await page.goto('http://127.0.0.1:8080/repos/admin/migrated-repo/issues');
    await expect(page.getByRole('heading', { name: 'Issues for admin/migrated-repo' })).toBeVisible();
    ...
    await page.goto('http://127.0.0.1:8080/repos/admin/migrated-repo/pulls');
    await expect(page.getByRole('heading', { name: 'Pull Requests for admin/migrated-repo' })).toBeVisible({ timeout: 10000 });
