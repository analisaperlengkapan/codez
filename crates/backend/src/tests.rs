...
mod tests {
    ...
    use shared::{
        ...
        CreateProtectedBranchOption, ProtectedBranch
    };
    ...
    #[tokio::test]
    async fn test_branch_protection_flow() {
        let app = api_router();

        // 1. Create Protected Branch
        let payload = CreateProtectedBranchOption {
            branch_name: "main".to_string(),
            enable_push: false,
            enable_force_push: false,
            required_approvals_to_merge: 1,
        };
        let response = app.clone()
            .oneshot(
                Request::builder()
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let body = axum::body::to_bytes(response.into_body(), usize::MAX).await.unwrap();
        let branches: Vec<ProtectedBranch> = serde_json::from_slice(&body).unwrap();
        assert!(branches.iter().any(|b| b.name == "main"));

        // 3. Delete Protected Branch
        let response = app.clone()
            .oneshot(
                Request::builder()
                    .method("DELETE")
                    .uri("/api/v1/repos/admin/codeza/branch_protections/main")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    #[tokio::test]
    async fn test_global_issue_search() {
        let app = api_router();

        // Create Issue 1: "Global Search Test"
        let payload = CreateIssueOption {
            title: "Global Search Test".to_string(),
            body: None,
        };
        let _ = app.clone()
            .oneshot(
        ...
        let response = app.clone()
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri("/api/v1/search/issues?q=Global")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
        ...
        assert!(issues.len() >= 1);
        assert!(issues.iter().any(|i| i.title.contains("Global")));
    }
}
