...
use shared::{
    ...
    IssueEvent, PullRequestEvent, PushEvent, RepoPulseStats, SecurityScanReport, SecurityVulnerability
};
...
pub async fn run_security_scan(
    ...
    if repo_id == 0 {
        return (StatusCode::NOT_FOUND, Json(SecurityScanReport {
            id: "".to_string(),
            repo_owner: owner,
            repo_name,
            score: 0,
    ...
    for ((r_id, branch, path), content) in files.iter() {
        if *r_id == repo_id && branch == &default_branch {
            ...
            if lower_content.contains("aws_secret_access_key") || lower_content.contains("BEGIN PRIVATE KEY") || lower_content.contains("ghp_") {
                vulnerabilities.push(SecurityVulnerability {
                    id: format!("SEC-{}", vul_id),
                    severity: "CRITICAL".to_string(),
                    title: "Hardcoded Secret / API Token Detected".to_string(),
                    description: format!("File '{}' contains potentially exposed secrets or private credentials.", path),
                    file_path: path.clone(),
                    line_no: 1,
                    recommendation: "Remove secret and use Environment Variables or Repository Secrets.".to_string(),
                });
                vul_id += 1;
            }

            // Rule 2: SQL Injection risks
            if lower_content.contains("select ") && lower_content.contains(" + ") && lower_content.contains("from ") {
                vulnerabilities.push(SecurityVulnerability {
                    id: format!("SEC-{}", vul_id),
                    severity: "HIGH".to_string(),
                    title: "Potential SQL Injection Vulnerability".to_string(),
                    description: format!("String concatenation in SQL query detected in file '{}'.", path),
                    file_path: path.clone(),
                    line_no: 1,
            ...
            if path.ends_with(".rs") && lower_content.contains("unsafe {") {
                vulnerabilities.push(SecurityVulnerability {
                    ...
                    description: format!("Unsafe Rust block used in '{}'. Ensure boundary checks are enforced.", path),
                    file_path: path.clone(),
                    line_no: 1,
                    recommendation: "Review unsafe block for potential memory safety violations.".to_string(),
                });
                vul_id += 1;
            }
        }
    ...
    let score = if vulnerabilities.is_empty() {
        ...
        100u8.saturating_sub((vulnerabilities.len() * 20) as u8).max(10)
    };
