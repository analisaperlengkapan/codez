...
pub async fn list_releases(
    ...
    let releases = state.releases.read().unwrap_or_else(|e| e.into_inner());
    ...
    filtered_releases.sort_by_key(|r| std::cmp::Reverse(r.id));
...
pub async fn create_release(
    ...
    let mut releases = state.releases.write().unwrap_or_else(|e| e.into_inner());
    // Generate safe ID
