...
use shared::{
    ...
    IssueFilterOptions, LoginOption, Notification, OAuth2Provider, PublicKey, PullRequest,
    RegisterOption, Repository, TwoFactor, User, UserSettingsOption,
};
...
pub async fn list_keys(State(state): State<AppState>) -> Json<Vec<PublicKey>> {
    let keys = state.keys.read().unwrap_or_else(|e| e.into_inner());
    Json(keys.clone())
}
...
pub async fn create_key(
    ...
    let mut keys = state.keys.write().unwrap_or_else(|e| e.into_inner());
...
pub async fn list_feeds(State(state): State<AppState>) -> Json<Vec<Activity>> {
    let feeds = state.activities.read().unwrap_or_else(|e| e.into_inner());
    // Return latest first
    let mut feeds = feeds.clone();
    feeds.sort_by_key(|f| std::cmp::Reverse(f.id));
    Json(feeds)
}

pub async fn list_gpg_keys(State(state): State<AppState>) -> Json<Vec<GpgKey>> {
    let keys = state.gpg_keys.read().unwrap_or_else(|e| e.into_inner());
    Json(keys.clone())
}
...
pub async fn create_gpg_key(
    ...
    let mut keys = state.gpg_keys.write().unwrap_or_else(|e| e.into_inner());
    let id = (keys.len() as u64) + 1;
    let key = GpgKey {
        id,
...
pub async fn delete_ssh_key(State(state): State<AppState>, Path(id): Path<u64>) -> StatusCode {
    let mut keys = state.keys.write().unwrap_or_else(|e| e.into_inner());
...
pub async fn delete_gpg_key(State(state): State<AppState>, Path(id): Path<u64>) -> StatusCode {
    let mut keys = state.gpg_keys.write().unwrap_or_else(|e| e.into_inner());
    if let Some(pos) = keys.iter().position(|k| k.id == id) {
        keys.remove(pos);
        StatusCode::NO_CONTENT
...
pub async fn list_oauth2_apps(State(state): State<AppState>) -> Json<Vec<OAuth2Application>> {
    let apps = state.oauth2_apps.lock().unwrap_or_else(|e| e.into_inner());
...
pub async fn create_oauth2_app(
    ...
    let mut apps = state.oauth2_apps.lock().unwrap_or_else(|e| e.into_inner());
...
pub async fn delete_oauth2_app(State(state): State<AppState>, Path(id): Path<u64>) -> StatusCode {
...
pub async fn list_followers(
    ...
    let followers_map = state.followers.read().unwrap_or_else(|e| e.into_inner());
    let follower_ids = followers_map
        .get(&target_user_id)
        .cloned()
        .unwrap_or_default();
...
pub async fn list_following(
    ...
    let following_map = state.following.read().unwrap_or_else(|e| e.into_inner());
    let following_ids = following_map
        .get(&target_user_id)
        .cloned()
        .unwrap_or_default();
...
pub async fn follow_user(
    ...
    if let Some(target_id) = target_id {
        ...
        let mut followers = state.followers.write().unwrap_or_else(|e| e.into_inner());
        ...
        let mut following = state.following.write().unwrap_or_else(|e| e.into_inner());
